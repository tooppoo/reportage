...
use crate::model::{
    ...
    OutputMatcher, RequiredLiteralKind, Step, TextLiteral, TextValueExpression, ValueLiteralKind,
    WorkspacePathError,
};
...
fn file_text_equals_accepts_string_literal() {
    ...
    match &file_exp.matcher {
        FileMatcher::TextEquals(TextValueExpression::Raw(TextLiteral::Quoted(value))) => {
...
fn file_text_equals_accepts_heredoc_literal() {
    ...
    match &file_exp.matcher {
        FileMatcher::TextEquals(TextValueExpression::Raw(TextLiteral::Heredoc(value))) => {
...
fn stdout_text_equals_accepts_string_literal() {
    ...
    match &block.expectations()[0] {
        Expectation::Stdout(OutputExpectation {
            matcher: OutputMatcher::TextEquals(TextValueExpression::Raw(TextLiteral::Quoted(value))),
        }) => assert_eq!(value, "hello\n"),
...
fn stderr_text_equals_accepts_heredoc_literal() {
    ...
    match &block.expectations()[0] {
        Expectation::Stderr(OutputExpectation {
            matcher:
                OutputMatcher::TextEquals(TextValueExpression::Raw(TextLiteral::Heredoc(value))),
...
fn stdout_text_equals_heredoc_parses_alongside_other_expectations() {
    ...
    assert!(matches!(
        ...
        Expectation::Stdout(OutputExpectation {
            matcher: OutputMatcher::TextEquals(TextValueExpression::Raw(TextLiteral::Heredoc(_))),
        })
    ));
