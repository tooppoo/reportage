...
- `mode` is optional. A `write` step without one creates a file that group and other can never access: its bits are requested as `0o600` and then reduced by the umask the reportage process runs under, so under an ordinary umask the file is exactly `0o600`. Unlike an explicit `mode`, this default is umask-dependent.
