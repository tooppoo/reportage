...
The rationale for the commands' internal boundaries is recorded in [ADR: Documentation Generation Command](../adr/20260723T070556Z_documentation-generation-command.md); the Markdown format's serialization decisions are recorded in [ADR: Markdown Documentation Format](../adr/20260723T143711Z_markdown-documentation-format.md); the split into two subcommands is recorded in [ADR: Reportage-Source Documentation Is Its Own Subcommand](../adr/20260907T230710Z_reportage-source-documentation-subcommand.md).
...
| --- | --- | --- |
| `reportage docs` | users of a product that is tested with reportage | product-facing documentation — *not implemented yet; see [Transitional state](#transitional-state)* |
...
- [Input patterns](#input-patterns) and [Source paths in the generated document](#source-paths-in-the-generated-document)
...
Everything else below — [Document structure](#document-structure), [Plain text serialization](#plain-text-serialization), and [Markdown serialization](#markdown-serialization), including [Source fences](#source-fences) — defines the Reportage-source projection, and is the contract of `reportage docs-reportage` alone.

### Transitional state

`reportage docs` currently produces the same Reportage-source document as `docs-reportage`, and switches to product-facing generation in a later change.
Until then it has no projection contract of its own: the two producing identical documents is a property of the migration, not a guarantee, and nothing may depend on it.
...
```console
reportage docs-reportage 'examples/**/*.repor' --out-dir generated/docs --format markdown --title 'Project documentation'
```

Generation aggregates the `document file` / `document case` metadata and the original `before_each` and case sources of every selected `.repor` file into generated documentation under `--out-dir`.
