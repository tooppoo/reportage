...
| --- | --- | --- |
| `reportage docs` | users of a product that is tested with reportage | product-facing documentation |
...
Everything below other than this section — source selection, display paths, the document title, the index file name, the input text policy, both serializations, and the output directory rules — is the contract of both subcommands.

**Transitional state:** `reportage docs` currently produces the same Reportage-source documentation as `docs-reportage`, and switches to product-facing generation in a later change.
The two producing identical documents is a property of the migration, not a guarantee; do not depend on it.
