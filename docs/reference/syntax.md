...
//
// Everything else — `mode = 0o755`, `0O755`, `0755`, `755`, `0o75`, `0o1000`,
// `0o888`, and symbolic chmod syntax such as `u+x` — fails this rule. Because
// the whole `(ws+ ~ write_mode)?` group is then skipped rather than partially
// matched, the unmatched text is next offered to the content position, where
// it is not a valid literal either, so every rejected spelling surfaces as a
// syntax error before execution rather than as a silently ignored modifier.
//
