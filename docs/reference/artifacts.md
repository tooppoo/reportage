...
CI validates each representative fixture run's `result.json` against that schema with the [`jsonschema`](https://docs.rs/jsonschema/) crate, inside `cargo nextest`. [`crates/reportage-cli/tests/run_result_fixtures.rs`](../../crates/reportage-cli/tests/run_result_fixtures.rs) additionally deserialises the manifest into the Rust structs of [`crates/reportage-cli/tests/run_result_fixtures/typed_consumer_model.rs`](../../crates/reportage-cli/tests/run_result_fixtures/typed_consumer_model.rs), marked `#[serde(deny_unknown_fields)]` and covering the full stable contract — every expectation kind, observation enum, and diagnostic shape — as a consumer compatibility test rather than as schema validation. See [`docs/adr/20260728T092956Z_json-contract-validation-policy.md`](../adr/20260728T092956Z_json-contract-validation-policy.md).

### Generated / checked / handwritten boundary

This document mixes three kinds of content, with different staleness guarantees:

- **Checked sections.** The JSON examples below marked with a `<!-- checked-against: ... -->` comment are verified byte-for-byte against fixture snapshots by `run_result_fixtures.rs::docs_artifacts_examples_match_their_fixture_snapshots`; they cannot silently drift from real output. Field lists, required/optional fields, and enums are checked against the schema for the shapes the representative fixtures produce, and against the whole stable contract by `run_result_fixtures.rs`'s typed consumer model; evidence references (layout, sizes, digests) are checked by `run_result_fixtures.rs::evidence_files_match_their_manifest_references` and [`e2e/artifacts/run-result-manifest.repor`](../../e2e/artifacts/run-result-manifest.repor).
