...
- [Documentation generation](docs-generation.md): the `reportage docs` and `reportage docs-reportage` subcommands, which audience each one serves, their input pattern rules, the product-facing and Reportage-source serialization contracts, and the output replacement guarantees. Their representative outputs are mechanically checked against the generated example documents under [`tests/fixtures/docs-product/`](../../tests/fixtures/docs-product/) and [`tests/fixtures/docs/`](../../tests/fixtures/docs/).
