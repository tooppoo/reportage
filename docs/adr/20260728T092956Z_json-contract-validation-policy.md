...
Coverage is therefore per site for the constraints that repeat. The case suite builds one base document instantiating every closed shape both contracts define, and generates from it: every object in that document must reject an undefined member, and every value must reject one of the wrong JSON type. A mutation test keeps the document and the schemas in step — for every `additionalProperties: false` in a contract, the schema is recompiled without that one keyword and some case must notice — so a closed object no case watches is a failure rather than a silent gap, however the schema reaches it. `oneOf` and the conditional are written out per site, four each.
