...
- The stable diagnostic code `parse.raw_block.shallow_indent` is unchanged (renaming it would be a breaking change per [`docs/diagnostics.md`](../diagnostics.md)'s compatibility policy, and this issue doesn't require one); only its prose description and the internal Rust enum variant name (`ParseError::ShallowHeredocIndent`) were updated.
