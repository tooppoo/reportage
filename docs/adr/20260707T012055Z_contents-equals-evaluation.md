...
- The `.reportage/runs/<id>/result.json` evidence schema gained `fileContentsEquals` / `stdoutContentsEquals` / `stderrContentsEquals` expectation kinds and an `expectedSource` field; this is additive and not currently pinned by an artifact schema stability contract (see docs/TBD.md — Artifact schema stabilization).
