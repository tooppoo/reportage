...
`parse.*` covers pest grammar syntax errors and parse-domain validation errors raised while constructing the AST. This includes a heredoc literal's indentation (`parse.heredoc_literal.shallow_indent`, applicable both to a `write` step's heredoc literal and to a `file ... contains` expectation's heredoc literal — see [`docs/semantics.md`](semantics.md) — Text literal), a document block's body rules (`parse.document_block.empty` / `.duplicate_field` / `.invalid_order`) and the `document file` placement rules (`parse.document_file.duplicate` / `.after_case` — see [`docs/semantics.md`](semantics.md) — Document block), a `write` step's workspace path safety validation (`semantic.workspace_path.*`, surfaced as a `ParseError` because `WorkspacePath::parse` runs during AST construction — see [`docs/semantics.md`](semantics.md) — Write step), a fixture reference literal's lexical validation (`semantic.fixture_reference.empty` / `.absolute` / `.dot_segment`, surfaced as a `ParseError` for the same reason, because `FixtureReference::parse` also runs during AST construction — see [`docs/semantics.md`](semantics.md) — Fixture reference value), and literal kind mismatches (`semantic.literal.kind_mismatch`, surfaced as a `ParseError` because argument positions check their literal kind during AST construction — see [`docs/semantics.md`](semantics.md) — Value literals). The `semantic.*` and `assertion.*` namespaces cover the semantic evaluator side and are defined in [`semantic-diagnostics.md`](semantic-diagnostics.md); this document does not define them. The `step.*` namespace covers runtime failures of side-effecting steps and is also defined in [`semantic-diagnostics.md`](semantic-diagnostics.md).
