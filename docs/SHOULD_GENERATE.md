...
This file records every document in this repository that is produced by a generator or protected by a mechanical drift check, plus the hand-written sections that are candidates for the same treatment. Generated documents are not confined to `docs/`. Per this repository's documentation policy, generated output is never hand-edited: change the source or the generator, then regenerate.

## Generated documents under `docs/`
...
## Generated documents outside `docs/`

| Path | Generator and source | Reason |
| --- | --- | --- |
| [`examples/README.md`](../examples/README.md) | `reportage docs-reportage` (`just examples-docs-gen`, drift-checked by `just examples-docs-check`), from [`examples/*.repor`](../examples/) | The index restates each example's `document file` / `document case` metadata and its case bodies; generating it from the executable examples keeps the published index and the scripts CI runs from diverging. The examples document reportage itself, so the generator is the Reportage-source one — see [Documentation generation](reference/docs-generation.md). |
...
## Mechanically checked documents under `docs/`
