...
1. **Is the behavior visible through the CLI, expressible as a `$` action and an `assert` block, and does asserting it not require inspecting internal Rust types?** Write a `.repor` self-test. See [Self-testing](self-testing.md).
2. **Does verifying the behavior require the self-testing harness itself, PATH shim resolution, the Cargo-built binary, coverage/subprocess routing, or exact artifact JSON / diagnostic structure?** Write a Rust integration test. See [Rust integration tests](rust-integration-tests.md).
3. **Is the behavior internal to a single component, parser output, AST shape, semantic evaluation, expectation evaluation, diagnostic construction, or artifact model, and does it not need a full CLI invocation to observe?** Write a Rust unit / focused test. See [Rust unit / focused tests](rust-unit-tests.md).
