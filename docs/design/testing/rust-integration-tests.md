...
- **Suite self-test runs.** That the `e2e/`, `examples/`, and `tests/fixtures/syntax/{valid,invalid}` suites run through the PATH overlay shim, with the outer suite invocation started by command name rather than a direct `cargo_bin` path; see the suite tests in `crates/reportage-cli/tests/self_test.rs` and [Self-testing](self-testing.md).
- **PATH shim resolution.** That a shim placed ahead of the inherited `PATH` resolves before any ambient binary of the same name, and that nested `reportage` invocations inside self-tests connect to the expected binary. See `shim_resolves_before_ambient_reportage` and `json_output_exposes_observed_inner_shim_invocations` in `crates/reportage-cli/tests/self_test.rs`.
