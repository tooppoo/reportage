...
- **Self-testing harness bootstrap.** That the harness resolves and invokes the Cargo-built `reportage` binary, not an installed one; see `crates/reportage-cli/tests/support/mod.rs` and `crates/reportage-cli/tests/self_test.rs`.
- **Suite self-test runs.** That the `e2e/`, `examples/`, and `tests/fixtures/syntax/{valid,invalid}` suites run through the PATH overlay shim, with the outer suite invocation started by command name rather than a direct `cargo_bin` path; see the suite tests in `crates/reportage-cli/tests/self_test.rs` and [self-testing.md](self-testing.md).
