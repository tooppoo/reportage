...
`reportage check <file> --format=json` does not exist yet. Do not suggest running it; see [common mistakes](common-mistakes.md).
...
- `processExitCode` is the reportage process's own exit code, separate from any exit code of the command a case ran. See [the exit code reference](../reference/exit-codes.md) for the full table and what each code means.
- `diagnostics` carries structured parse/validation/runtime errors, each with a stable `category` and `code`. Read [the parse diagnostics reference](../reference/diagnostics.md) for parser/validator codes and [the semantic and assertion diagnostics reference](../reference/semantic-diagnostics.md) for `semantic.*` / `assertion.*` / `step.*` codes before guessing at what a code means from its name alone.
...
The full contract for this document, including every field's exact shape, is [the JSON execution report contract](../../spec/output/json-report/README.md) and its schema at [`spec/output/json-report/schema.json`](../../spec/output/json-report/schema.json). Do not rely on this document's summary above for anything beyond a first read.

## Artifacts

Every run also writes an artifact bundle under `.reportage/`, described in [the artifacts reference](../reference/artifacts.md). Its manifest contract is [the run result artifact contract](../../spec/artifacts/run-result/README.md) and [`spec/artifacts/run-result/schema.json`](../../spec/artifacts/run-result/schema.json). Use the artifact bundle, not memory of the stdout JSON, when you need evidence from the run after the fact.
